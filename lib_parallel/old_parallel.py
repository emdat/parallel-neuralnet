...
class parallel_network(object):
    ...
    def sgd(self, X, y, num_examples, num_epochs, test_data, mini_batch_sz, learning_rate, reg_lambda=0.01):
        ...
        accured_db = [0] * (self.num_layers-1)
        
        # Set up windows for shared memory space
        win_weights = []
        win_biases = []
        if rank == 0:
            for l in range(1, self.num_layers):
                win_weights.append(MPI.Win.Create(self.weights[l-1], comm=comm))
                win_biases.append(MPI.Win.Create(self.biases[l-1], comm=comm))        
        else:
            for l in range(1, self.num_layers):
                win_weights.append(MPI.Win.Create(None, comm=comm))        
                win_biases.append(MPI.Win.Create(None, comm=comm))
        ...
        for epoch in xrange(num_epochs):
            ...
            if rank == 0:
                ...
                my_X, my_y = shuffle(my_X, my_y, random_state=0)
                ...
                for mb_x, mb_y in zip(mini_batches_x, mini_batches_y):
                    if step % nfetch == 0:
                        for l in range(1, self.num_layers):
                            print "hello"
                            win_weights[l-1].Lock(rank=0)
                            win_weights[l-1].Get(origin=self.weights[l-1], target_rank=0)
                            win_biases[l-1].Get(origin=self.biases[l-1], target_rank=0)
                            win_weights[l-1].Unlock(rank=0)
                    ...
                    if step % npush == 0:
                        for l in range(1, self.num_layers):
                            win_weights[l-1].Lock(MPI_LOCK_EXCLUSIVE, 0)
                            win_weights[l-1].Accumulate(accrued_dw[l-1], 0, op=add_grad)
                            win_biases[l-1].Accumulate(accrued_db[l-1], 0, op=add_grad)
                            win_weights[l-1].Unlock(0)
                        accrued_dw = [0] * (self.num_layers-1)
                        accured_db = [0] * (self.num_layers-1)
